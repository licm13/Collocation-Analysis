...
- TCH: Three-Cornered Hat (Classic 3-way)
- EIVD: Extended IVD (3-way with error correlation)
- EC: Extended Collocation (4-way)
- BTC: Bayesian Triple Collocation (3-way, complex, time-varying)
- BTCH: Bayesian Three-Cornered Hat (3-way, simple, constant error)
"""
...
METHOD_COLORS = {
    ...
    'IVS': COLORS['orange'],
    'TC': COLORS['green'],
    'TCH': COLORS['pink'],  # Added
    'EIVD': COLORS['red'],
    'EC': COLORS['purple'],
    'BTC': COLORS['brown'],
    'BTCH': COLORS['gray'], # Added
}

# ============================================================================
# Data Simulation Functions
# (Unchanged from your file - keeping them for completeness)
# ============================================================================
...
def simulate_scenario_1_ideal(n=500, seed=42):
    ...
    return {
        ...
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Ideal Case', 'description': 'Independent errors, constant variance'
    }
...
def simulate_scenario_2_correlated(n=500, seed=43):
    ...
    return {
        'data': data, 'truth': truth, 'rmse_true': rmse_true,
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Correlated Errors', 'description': 'Products 2-3 share common error source'
    }
...
def simulate_scenario_3_timevarying(n=500, seed=44):
    ...
    return {
        'data': data, 'truth': truth, 'rmse_true': rmse_true,
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Time-Varying Errors', 'description': 'Heteroscedastic errors with seasonal pattern'
    }
...
def simulate_scenario_4_biased(n=500, seed=45):
    ...
    truth = 0.2 + 0.15 * np.sin(t) + 0.1 * np.sin(2*t)
    rmse_true = np.array([0.02, 0.03, 0.04, 0.025])
    bias_add = np.array([0.0, 0.05, -0.04, 0.03])
    bias_mul = np.array([1.0, 1.2, 0.8, 1.15])
    products = []
    for i in range(4):
        error = np.random.normal(0, rmse_true[i], n)
        product = bias_add[i] + bias_mul[i] * truth + error
    ...
    return {
        'data': data, 'truth': truth, 'rmse_true': rmse_true,
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Systematic Biases', 'description': 'Strong additive and multiplicative biases'
    }
...
def simulate_scenario_5_heavytail(n=500, seed=46):
    ...
    return {
        'data': data, 'truth': truth, 'rmse_true': rmse_true,
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Heavy-Tailed Errors', 'description': 'Student-t errors with outliers (df=3)'
    }
...
def simulate_scenario_6_realistic(n=500, seed=47):
    ...
    return {
        'data': data, 'truth': truth, 'rmse_true': rmse_true,
        'bias_add': bias_add, 'bias_mul': bias_mul,
        'name': 'Realistic Case', 'description': 'Combined: time-varying + correlated + biased'
    }

# ============================================================================
# Method Application Functions (MODIFIED)
# ============================================================================
...
def apply_classical_methods(scenario):
    ...
    try:
        ...
        results['IVD'] = {
            'rmse': rmse_ivd, 'rho2': rho2, 'n_products': 2, 'success': True
        }
    ...
    try:
        ...
        results['IVS'] = {
            'rmse': RMSE, 'rho2': rho2, 'n_products': 2, 'success': True
        }
    except Exception as e:
    ...
    try:
        ...
        results['TC'] = {
            'rmse': rmse_tc, 'SNR': SNR, 'rho2': rho2, 'fMSE': fMSE,
            'n_products': 3, 'success': True
        }
    except Exception as e:
        results['TC'] = {'success': False, 'error': str(e)}

    # TCH (3-way) - Added
    # Note: Using 'tch' alias which points to the same 'tc' implementation.
    try:
        tri = data[:3, :].T
        EeeT, SNR, rho2, fMSE = tch(tri) # Using tch alias
        rmse_tch = np.sqrt(np.diag(EeeT))
        results['TCH'] = {
            'rmse': rmse_tch, 'SNR': SNR, 'rho2': rho2, 'fMSE': fMSE,
            'n_products': 3, 'success': True
        }
    except Exception as e:
        results['TCH'] = {'success': False, 'error': str(e)}
    ...
    try:
        ...
        results['EIVD'] = {
            'rmse': rmse_eivd, 'SNR': SNR, 'rho2': rho2, 'fMSE': fMSE,
            'L': L, 'n_products': 3, 'success': True
        }
    except Exception as e:
    ...
    if n_products >= 4:
        try:
            ...
            if len(ec_results) > 0:
                ...
                results['EC'] = {
                    'rmse': rmse_ec, 'SNR': best.SNR[0], 'rho2': best.rho2[0],
                    'fMSE': best.fMSE[0], 'n_products': 4, 'success': True
                }
            else:
...
def apply_bayesian_method(scenario, niter=1000, nadvi=50000):
    """Apply Bayesian Triple Collocation (BTC - complex) to a scenario."""
    ...
    try:
        ...
        return {
            'rmse': rmse_mean, 'rmse_std': rmse_std, 'rmse_quantiles': rmse_quantiles,
...
def apply_bayesian_tch_method(scenario, niter=1000, nadvi=50000):
    """Apply Bayesian Three-Cornered Hat (BTCH - simple) to a scenario."""
    if not BAYESIAN_TCH_AVAILABLE:
        return {'success': False, 'error': 'PyMC3 not available for BTCH'}
    
    try:
        data = scenario['data'][:3, :]  # Use first 3 products
        btch = BayesianTCH(data) # Use the new class
        btch.run_inference(niter=niter, nadvi=nadvi, nchains=1, seed=42)
        rmse_mean, rmse_std, rmse_quantiles = btch.get_error_estimates()
        # This simple model doesn't estimate calibration params, so we fill Nones
        return {
            'rmse': rmse_mean, 'rmse_std': rmse_std, 'rmse_quantiles': rmse_quantiles,
            'bias_add': None, 'bias_mul': None,
            'n_products': 3, 'success': True
        }
    except Exception as e:
        return {'success': False, 'error': str(e)}
...
def plot_scenario_comparison(scenario, results, save_path=None):
    ...
    (Updated to include TCH and BTCH)
    """
    ...
    methods = ['IVD', 'IVS', 'TC', 'TCH', 'EIVD', 'EC', 'BTC', 'BTCH']
    ...
    for method in methods:
        if method in results and results[method]['success'] and 'rho2' in results[method]:
            ...
            for i in range(n_prod):
                ...
                colors_r.append(METHOD_COLORS.get(method, 'gray'))
    ...
    # ---- Bottom left: Error distributions ----
    ...
    for method in methods:
        if method in results and results[method]['success']:
            ...
            for i in range(n_prod):
                ...
                colors_re.append(METHOD_COLORS.get(method, 'gray'))
...
def plot_overall_comparison(all_results, save_path=None):
    ...
    methods = ['IVD', 'IVS', 'TC', 'TCH', 'EIVD', 'EC', 'BTC', 'BTCH'] # Updated list
...
def create_method_comparison_table(all_results):
    ...
    for scenario_name in scenarios:
        ...
        print(f"True RMSE: {np.array2string(scenario['rmse_true'], formatter={'float_kind':lambda x: '%.4f' % x})}")
        print(f"True add. bias: {np.array2string(scenario['bias_add'], formatter={'float_kind':lambda x: '%.4f' % x})}")
        print(f"True mul. bias: {np.array2string(scenario['bias_mul'], formatter={'float_kind':lambda x: '%.4f' % x})}")
...
def main():
    ...
    print("="*100)
    print("\nComparing: IVD, IVS, TC, TCH, EIVD, EC, BTC, BTCH")
    print("Across 6 scenarios with publication-quality figures.\n")
    ...
    for scenario in scenarios:
        ...
        print("  Applying classical methods (IVD, IVS, TC, TCH, EIVD, EC)...")
        results = apply_classical_methods(scenario)

        # Apply Bayesian method (BTC - complex)
        if BAYESIAN_AVAILABLE:
            print("  Applying Bayesian method (BTC - complex)...")
            results['BTC'] = apply_bayesian_method(scenario, niter=500, nadvi=30000)
        else:
            print("  Skipping BTC (PyMC3 not available)")
            results['BTC'] = {'success': False, 'error': 'PyMC3 not available'}

        # Apply Bayesian TCH method (BTCH - simple)
        if BAYESIAN_TCH_AVAILABLE:
            print("  Applying Bayesian method (BTCH - simple)...")
            results['BTCH'] = apply_bayesian_tch_method(scenario, niter=500, nadvi=30000)
        else:
            print("  Skipping BTCH (PyMC3 not available)")
            results['BTCH'] = {'success': False, 'error': 'PyMC3 not available'}
    ...
    print("\n(plt.show() commented out. Run script directly to see plots.)")
